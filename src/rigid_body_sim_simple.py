...
class RigidBodySimulation:
    ...
    def _resolve_ground_contact(self, body: RigidBody) -> None:
        ...
        min_half_extent = 0.01 * min(body.size)
...
def demo_scene() -> List[RigidBody]:
    size = (0.6, 0.6, 0.6)
    tilt_x = quat_from_axis_angle((1.0, 0.0, 0.0), math.radians(25.0))
    tilt_z = quat_from_axis_angle((0.0, 0.0, 1.0), math.radians(35.0))
    orientation = quat_normalize(quat_multiply(tilt_z, tilt_x))

    # Place the center so exactly one corner rests on the ground plane.
    rotation = quat_to_matrix(orientation)
    local_vertices = box_vertices(size)
    rotated_vertices = [mat3_mul_vec(rotation, vertex) for vertex in local_vertices]
    min_y = min(vertex[1] for vertex in rotated_vertices)
    position = (0.0, -min_y, 0.0)
...
def main() -> None:
    ...
    sim.run()
    output_dir = Path("output_new/obj_frames")
