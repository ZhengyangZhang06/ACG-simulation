...
class WCSPHSolver(SPHBase):
    def __init__(self, particle_system):
        ...
        self.d_velocity = ti.Vector.field(self.ps.dim, dtype=float, shape=self.ps.particle_max_num)

    @ti.kernel
    def compute_densities(self):
        for p_i in range(self.ps.particle_num[None]):
            x_i = self.ps.x[p_i]
            self.ps.density[p_i] = self.ps.m_V * self.cubic_kernel(0.0)
    ...
    def compute_non_pressure_forces(self):
        for p_i in range(self.ps.particle_num[None]):
            ...
            for j in range(self.ps.particle_neighbors_num[p_i]):
                ...
                d_v += self.surface_tension_force(p_i, p_j, x_i - x_j)
            self.d_velocity[p_i] = d_v
    ...
    def advect(self):
        ...
        for p_i in range(self.ps.particle_num[None]):
            self.d_velocity[p_i] = ti.Vector([0.0 for _ in range(self.ps.dim)])
