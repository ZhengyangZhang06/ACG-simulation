...
def is_rigid_body_sim(scene_path):
    """Check if scene is a rigid body simulation."""
    import json
    with open(scene_path, 'r') as f:
        config = json.load(f)
    return config.get("Configuration", {}).get("simulationType") == "rigid_body"

# Only initialize taichi for fluid simulations
import sys
if len(sys.argv) > 1:
    for i, arg in enumerate(sys.argv):
        if arg in ['--scene_file', '--scene'] and i + 1 < len(sys.argv):
            scene_file = sys.argv[i + 1]
            if not is_rigid_body_sim(scene_file):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='SPH Taichi / Rigid Body Simulation')
    ...
    simulation_type = config.get_cfg("simulationType", "fluid")
    
    if simulation_type == "rigid_body":
        # Rigid body simulation mode
        print("RIGID BODY SIMULATION MODE")
        
        from materials.rigid import create_simulation_from_config
        
        # Create and run simulation
        sim = create_simulation_from_config(config)
        print(f"Starting rigid body simulation: {len(sim.bodies)} bodies, {sim.config.duration}s")
        sim.run()
        
        # Export OBJ sequence
        output_dir = Path(f"output/{scene_name}/mesh_output")
        sim.export_obj_sequence(output_dir)
        print(f"Exported {len(sim.history)} frames to {output_dir}")
        
        # Export history JSON
        history_path = Path(f"output/{scene_name}/simulation_history.json")
        sim.export_history(history_path)
        print(f"Exported simulation history to {history_path}")

        print("Rigid body simulation complete!")
        
    else:
        # Original fluid simulation mode
